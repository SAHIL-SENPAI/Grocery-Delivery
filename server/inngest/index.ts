...
export const inngest = new Inngest({ id: "grocery-delivery5" });
...
const checkLowStock = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-low-stock-email",async()=>{
        ...
        // used nodemailer to send email messages to admins 
        await sendEmail({
          to:adminEmails.join(","),
...
const sendMonthlyOffers = inngest.createFunction({
  ...
  let sentCount = 0;
  // send in batches of 10 to avoid overwhelming main server;
  const batchSize  = 10;
...
const autoAssignRider = inngest.createFunction({
  ...
  // wait 5 mins before before attempting assignment
  ...
  const result = await step.run("assign-rider",async()=>{
    ...
    // skip if order dosent exist,cancelled,already assigned
    ...
    // find an acive rider not currently delivering
