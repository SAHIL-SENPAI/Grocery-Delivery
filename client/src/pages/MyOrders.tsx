            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                    {tabs.map((tab)=>(
                        ...
                        whitespace-nowrap transition-colors ${activetab === tab ? "bg-app-green text-white"
                ...
                {loading?(
                    ...
                    <div className="space-y-4">
                         {orders.map((order)=>(
                            ...
                            className="block max-w-4xl bg-white rounded-2xl p-5
                            hover:shadow transition-all">
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    {order.items.slice(0,4).map((item,i)=>(
                                        <img className="size-12 sm:size-16 rounded-lg object-cover
                                        border border-app-border" 
                                        key={i} src={item.image} alt={item.name}/>
                                    ))}
                                    {
                                        order.items.length > 4 && 
                                        <div className="size-12 sm:size-16 rounded-lg bg-app-cream
                                        flex-center text-xs font-semibold text-app-text-light">
                                            +{order.items.length-4}
                                        </div>
                                    }
                                </div>
