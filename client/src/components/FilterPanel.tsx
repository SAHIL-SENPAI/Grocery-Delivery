...
const FilterPanel = ({categories,category,minPrice,maxPrice,updateFilter,clearFilter,hasFilters}:any) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
            ...
            <div className="space-y-1.5">
                {categoriesWithAll.map((cat:any)=>(
                    <button key={cat.slug} onClick={()=>updateFilter("category",cat.slug)} className={`block w-full text-left px-3 py-2 text-sm rounded-md
        ...
        {hasFilters && (
            <button
            onClick={clearFilter} className="w-full py-2 text-sm text-app-error hover:bg-red-50
