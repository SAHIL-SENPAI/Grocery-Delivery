                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    ...
                    <div className="flex gap-3">
                        {footerData.brand.socials.map((social,i)=>(
                            <a key={i} className="size-9 rounded-lg bg-white/10
                             flex-center hover:bg-white/2" href={social.link}><social.icon className="size-4"/></a>
                        ))}
                ...
                sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex gap-4">
                        {footerData.bottom.links.map((link,i)=>(
                            <a className="text-xs text-white/50
                            hover:text-white/70" key={i} href={link.href}>{link.label}</a>
                        ))}
                    </div>
